# coding: utf-8

"""
    merged spec

    merged spec

    The version of the OpenAPI document: 1.0.0
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from xero_python.models.benefit import Benefit  # noqa: E501


class TestBenefit(unittest.TestCase):
    """Benefit unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> Benefit:
        """Test Benefit
        include_option is a boolean, when False only required
        params are included, when True both required and
        optional params are included"""
        # uncomment below to create an instance of `Benefit`
        """
        model = Benefit()  # noqa: E501
        if include_optional:
            return Benefit(
                id = '',
                name = '',
                category = 'StakeholderPension',
                liability_account_id = '',
                expense_account_id = '',
                standard_amount = 1.337,
                percentage = 1.337,
                calculation_type = 'FixedAmount',
                current_record = True,
                subject_to_nic = True,
                subject_to_pension = True,
                subject_to_tax = True,
                is_calculating_on_qualifying_earnings = True,
                show_balance_to_employee = True
            )
        else:
            return Benefit(
                name = '',
                category = 'StakeholderPension',
                liability_account_id = '',
                expense_account_id = '',
                percentage = 1.337,
                calculation_type = 'FixedAmount',
        )
        """

    def testBenefit(self):
        """Test Benefit"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)


if __name__ == "__main__":
    unittest.main()
