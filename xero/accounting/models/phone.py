...
class Phone(BaseModel):
    openapi_types = {"phone_type": "str", "phone_number": "str", "phone_area_code": "str", "phone_country_code": "str"}
    attribute_map = {
    ...
    def __init__(self, phone_type=None, phone_number=None, phone_area_code=None, phone_country_code=None):
    ...
    def phone_type(self, phone_type):
        allowed_values = ["DEFAULT", "DDI", "MOBILE", "FAX", "OFFICE", "None"]
        if phone_type:
            if phone_type not in allowed_values:
                raise ValueError(f"Invalid value for `phone_type` ({phone_type}), must be one of {allowed_values}")
