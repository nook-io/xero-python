...
class EmployeeLeaveSetupObject(BaseModel):
    ...
    leave_setup: Optional[EmployeeLeaveSetup] = Field(default=None, alias="leaveSetup")
    __properties = ["pagination", "problem", "leaveSetup"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> EmployeeLeaveSetupObject:
    ...
    def to_dict(self):
        ...
        # override the default output from pydantic by calling `to_dict()` of leave_setup
        if self.leave_setup:
            _dict["leaveSetup"] = self.leave_setup.to_dict()
        return _dict

    @classmethod
    def from_dict(cls, obj: dict) -> EmployeeLeaveSetupObject:
        """Create an instance of EmployeeLeaveSetupObject from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return EmployeeLeaveSetupObject.parse_obj(obj)

        _obj = EmployeeLeaveSetupObject.parse_obj(
            {
                ...
                "leave_setup": EmployeeLeaveSetup.from_dict(obj.get("leaveSetup"))
                if obj.get("leaveSetup") is not None
                else None,
            }
        )
        return _obj
