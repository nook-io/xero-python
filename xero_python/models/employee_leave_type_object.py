...
class EmployeeLeaveTypeObject(BaseModel):
    ...
    leave_type: Optional[EmployeeLeaveType] = Field(default=None, alias="leaveType")
    __properties = ["pagination", "problem", "leaveType"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> EmployeeLeaveTypeObject:
    ...
    def to_dict(self):
        ...
        # override the default output from pydantic by calling `to_dict()` of leave_type
        if self.leave_type:
            _dict["leaveType"] = self.leave_type.to_dict()
        return _dict

    @classmethod
    def from_dict(cls, obj: dict) -> EmployeeLeaveTypeObject:
        """Create an instance of EmployeeLeaveTypeObject from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return EmployeeLeaveTypeObject.parse_obj(obj)

        _obj = EmployeeLeaveTypeObject.parse_obj(
            {
                ...
                "leave_type": EmployeeLeaveType.from_dict(obj.get("leaveType"))
                if obj.get("leaveType") is not None
                else None,
            }
        )
        return _obj
