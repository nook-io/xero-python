...
class EmployeeLeaves(BaseModel):
    ...
    leave: Optional[conlist(EmployeeLeave1)] = None
    __properties = ["pagination", "problem", "leave"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> EmployeeLeaves:
    ...
    def to_dict(self):
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in leave (list)
        _items = []
        if self.leave:
            for _item in self.leave:
                if _item:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> EmployeeLeaves:
        """Create an instance of EmployeeLeaves from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return EmployeeLeaves.parse_obj(obj)

        _obj = EmployeeLeaves.parse_obj(
            {
                ...
                "leave": [EmployeeLeave1.from_dict(_item) for _item in obj.get("leave")]
                if obj.get("leave") is not None
                else None,
            }
        )
        return _obj
