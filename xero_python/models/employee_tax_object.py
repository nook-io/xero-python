...
class EmployeeTaxObject(BaseModel):
    ...
    employee_tax: Optional[EmployeeTax] = Field(default=None, alias="employeeTax")
    __properties = ["pagination", "problem", "employeeTax"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> EmployeeTaxObject:
    ...
    def to_dict(self):
        ...
        # override the default output from pydantic by calling `to_dict()` of employee_tax
        if self.employee_tax:
            _dict["employeeTax"] = self.employee_tax.to_dict()
        return _dict

    @classmethod
    def from_dict(cls, obj: dict) -> EmployeeTaxObject:
        """Create an instance of EmployeeTaxObject from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return EmployeeTaxObject.parse_obj(obj)

        _obj = EmployeeTaxObject.parse_obj(
            {
                ...
                "employee_tax": EmployeeTax.from_dict(obj.get("employeeTax"))
                if obj.get("employeeTax") is not None
                else None,
            }
        )
        return _obj
