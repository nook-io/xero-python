...
class Reimbursements(BaseModel):
    ...
    reimbursements: Optional[conlist(Reimbursement)] = None
    __properties = ["pagination", "problem", "reimbursements"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> Reimbursements:
    ...
    def to_dict(self):
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in reimbursements (list)
        _items = []
        if self.reimbursements:
            for _item in self.reimbursements:
                if _item:
                    _items.append(_item.to_dict())
            _dict["reimbursements"] = _items
        return _dict

    @classmethod
    def from_dict(cls, obj: dict) -> Reimbursements:
        """Create an instance of Reimbursements from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return Reimbursements.parse_obj(obj)

        _obj = Reimbursements.parse_obj(
            {
                ...
                "reimbursements": [Reimbursement.from_dict(_item) for _item in obj.get("reimbursements")]
                if obj.get("reimbursements") is not None
                else None,
            }
        )
        return _obj
